...
void __global__ MorphologicalReconstructionKernel(GlobalQueue* gq, volatile unsigned char* marker, unsigned char* mask, 
            size_t pitchMarker, size_t pitchMask, int size, int* memLeak, unsigned long long* times)
{
    ...
    if(tid==0) {
        ...
        atomicAdd((int*)(&(gq->activeBlocksCount)), 1);
    }

    __syncthreads();

    // Processamento da fila global
    //while((readCount = readFromGlobalQueue(gq, &globalQueueReadBuffer, 1, currentActive)) or atomicCAS(&(gq->activeBlocksCount), -2, -2)) {
    auxStart = clock64();
    while((readCount = readFromGlobalQueue(gq, &globalQueueReadBuffer, 1, currentActive)) or gq->activeBlocksCount) {
        ...
        for(i=0; i<MEGAPIXEL_SIZE; i++) {
            markerRow = (volatile unsigned char*)((char*)marker + (MEGAPIXEL_SIZE*globalQueueReadBuffer.x + i) * pitchMarker);
        ...
        if(globalQueueReadBuffer.x > 0) {
            ...
            markerRow = (volatile unsigned char*)((char*)marker + (MEGAPIXEL_SIZE*globalQueueReadBuffer.x - 1) * pitchMarker);
        ...
        if(globalQueueReadBuffer.y < (size/MEGAPIXEL_SIZE)-1) {
            markerRow = (volatile unsigned char*)((char*)marker + (MEGAPIXEL_SIZE*globalQueueReadBuffer.x + tid) * pitchMarker);
        ...
        for(i=0; i<MEGAPIXEL_SIZE; i+=4) {
            markerRow = (volatile unsigned char*)((char*)marker + (MEGAPIXEL_SIZE*globalQueueReadBuffer.x + i + rowOffset) * pitchMarker);
        ...
        // Armazenamos o próximo tempo de início de leitura
        auxStart = clock64();
    }
            ...
            size_t pitchMarker, size_t pitchMask, int size)
{
    ...
    // Iniclizamos o contador
    cudaMalloc(&d_times, 5*sizeof(unsigned long long));
    cudaMemset(d_times, 0, 5*sizeof(unsigned long long));
    ...
    do{
        ...
        MorphologicalReconstructionKernel<<<gridSize, blockSize>>>(gq, marker, mask, pitchMarker, pitchMask, size, dMemLeak, d_times);
    ...
    //printf("QUEUE READ\tMP_FETCH\tMP_PROCESSING\tMP_DUMP\tQUEUE_WRITE\tTIME\n");
    //printf("%llu\t%llu\t%llu\t%llu\t%llu\t", h_times[QUEUE_READ], h_times[MP_FETCH], h_times[MP_PROCESSING], h_times[MP_DUMP], h_times[QUEUE_WRITE]);
