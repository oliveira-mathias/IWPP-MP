...
__device__ unsigned int atomicCASUShort2(ushort2* address, ushort2 compare, ushort2 val) {
    unsigned int* address_as_uint = (unsigned int*) address;
    unsigned int* compare_address_as_uint = (unsigned int*) &compare;
    unsigned int* val_address_as_uint = (unsigned int*) &val;

    return atomicCAS(address_as_uint, *compare_address_as_uint, *val_address_as_uint);
}

__device__ int insertIntoGlobalQueue(GlobalQueue* gq, ushort2* data, int n, bool atomic) {
    ...
    if(tid==0) {
        ...
        if(((*(gq->globalSize)) + n) <= MP_GLOBAL_QUEUE_SIZE) {
            ...
            printf("toWrite: %d, gqSize: %d, tamanhoDaFila: %d\n", n, *(gq->globalSize), MP_GLOBAL_QUEUE_SIZE);
        }
    }
    ...
    if(tid < n) {
        if(!atomic) {
            ...
            atomicCASUShort2(&(gq->globalQueue[writeOffset + tid]), gq->globalQueue[writeOffset + tid], data[tid]);
        }
    }
...
__global__ void initFillGlobalQueue(GlobalQueue* gq) {
    unsigned short xCoord = blockDim.x * blockIdx.x + threadIdx.x;
    unsigned short yCoord = blockDim.y * blockIdx.y + threadIdx.y;
...
__device__ int readFromGlobalQueue(GlobalQueue* gq, ushort2* readBuffer, int n, bool& currentActive) {
...
__global__ void allocGlobalQueue(GlobalQueue* gq) {
    ...
    cudaMalloc(&dataPtr, MP_GLOBAL_QUEUE_SIZE);
